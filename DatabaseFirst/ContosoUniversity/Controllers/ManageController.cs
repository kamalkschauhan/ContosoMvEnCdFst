...
namespace ContosoUniversity.Controllers
{
    ...
    public class ManageController : Controller
    {
        ...
        {
            var code = await UserManager.GenerateChangePhoneNumberTokenAsync(User.Identity.GetUserId(), phoneNumber).ConfigureAwait(false);
        ...
        public async Task<ActionResult> VerifyPhoneNumber(VerifyPhoneNumberViewModel model)
        {
            ...
            var result = await UserManager.ChangePhoneNumberAsync(User.Identity.GetUserId(), model.PhoneNumber, model.Code).ConfigureAwait(false);
            if (result.Succeeded)
            {
                var user = await UserManager.FindByIdAsync(User.Identity.GetUserId()).ConfigureAwait(false);
                if (user != null)
        ...
        public async Task<ActionResult> ManageLogins(ManageMessageId? message)
        {
            ...
            var userLogins = await UserManager.GetLoginsAsync(User.Identity.GetUserId()).ConfigureAwait(false);
        ...
        {
            var loginInfo = await AuthenticationManager.GetExternalLoginInfoAsync(XsrfKey, User.Identity.GetUserId()).ConfigureAwait(false);
            if (loginInfo == null)
            {
                return RedirectToAction("ManageLogins", new { Message = ManageMessageId.Error });
            }
            var result = await UserManager.AddLoginAsync(User.Identity.GetUserId(), loginInfo.Login).ConfigureAwait(false);
